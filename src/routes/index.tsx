...
const STATS = [
  ...
  { n: "1", l: "RERA Registered Firm", i: "✅" },
];

function Home() {
  const { data } = useProperties();
  const featured = data.filter((p) => p.is_featured);
  const marquee = featured.length > 0 ? featured : data.slice(0, 3);
  ...
  return (
    <SiteLayout>
      ...
      <section
        ...
        style={{
          backgroundImage: `linear-gradient(rgba(10,15,30,0.85), rgba(10,15,30,0.95)), url(${office.url})`,
          backgroundSize: "cover",
          backgroundPosition: "center",
        }}
      >
        <div className="mx-auto max-w-4xl px-4 py-16 text-center animate-fade-in">
          <span className="inline-block rounded-full border border-emerald/40 bg-emerald/10 px-4 py-1.5 text-xs font-semibold text-emerald">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-lg text-grey">
            Trusted real estate consultancy helping families buy, sell &amp; rent in Kalwa, Thane
            since over a decade
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <Link to="/properties" className="rounded-full bg-emerald px-7 py-3 font-semibold text-white transition-transform hover:scale-105">
              Explore Properties →
            </Link>
            <a href={WHATSAPP_LINK} target="_blank" rel="noreferrer" className="rounded-full border border-gold px-7 py-3 font-semibold text-gold transition-colors hover:bg-gold/10">
              WhatsApp Us
            </a>
          </div>
        ...
        <div className="marquee-wrap mt-12 overflow-hidden">
          <div className="marquee-track gap-6">
            {[...marquee, ...marquee].map((p, i) => (
              <div key={i} className="w-[330px] shrink-0">
      ...
      <section className="bg-navy py-20">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-5">
          <div className="lg:col-span-3">
            ...
            <Link to="/about" className="mt-6 inline-block rounded-full bg-emerald px-6 py-2.5 font-semibold text-white transition-transform hover:scale-105">
              Read More About Us →
            </Link>
          </div>
          <div className="grid grid-cols-2 gap-4 lg:col-span-2">
            {STATS.map((s) => (
              <div key={s.l} className="glass flex flex-col items-center justify-center rounded-xl p-6 text-center">
      ...
      <section className="bg-navy-light py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="text-center">
            ...
            <h2 className="mt-2 font-display text-3xl font-bold text-white sm:text-4xl">What Our Clients Say</h2>
          </div>
      ...
      <section className="py-16" style={{ background: "linear-gradient(135deg, #10b981, #f59e0b)" }}>
        <div className="mx-auto max-w-4xl px-4 text-center">
          <h2 className="font-display text-3xl font-bold text-white sm:text-4xl">Ready to Find Your Perfect Property?</h2>
          <p className="mt-3 text-white/90">Call us today for a free consultation</p>
          <div className="mt-7 flex flex-wrap justify-center gap-4">
            <a href={`tel:${CONTACT.phones[0]}`} className="rounded-full bg-navy px-6 py-3 font-semibold text-white transition-transform hover:scale-105">📞 Call Now: {CONTACT.phones[0]}</a>
            <a href={WHATSAPP_LINK} target="_blank" rel="noreferrer" className="rounded-full bg-navy px-6 py-3 font-semibold text-white transition-transform hover:scale-105">💬 WhatsApp Us</a>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-5xl px-4 text-center">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            {["SBI", "HDFC", "ICICI", "LIC", "AXIS BANK"].map((b) => (
              <span key={b} className="glass rounded-full px-6 py-2.5 text-sm font-semibold text-grey">{b}</span>
            ))}
          </div>
        </div>
      </section>
  ...
  return (
    <div className="glass rounded-2xl p-7 transition-all hover:glow-border">
      ...
      <div className="mt-5 flex items-center gap-3">
        <div className="flex h-11 w-11 items-center justify-center rounded-full bg-emerald font-bold text-white">{name[0]}</div>
        <div>
